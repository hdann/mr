...
module.exports.run = async function({ api, args, event, permssion }) {
  ...
  module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
    ...
    if (!fs.existsSync(__dirname +
      `/lq/utm-avobold1.ttf`)) {
      let getfont = (await axios.get(`https://drive.google.com/u/0/uc?id=1bh2v2VPcrHv5CNCWPQzuv-j6cxJ7EufK&export=download`, { responseType: "arraybuffer" })).data;
      fs.writeFileSync(__dirname + `/lq/utm-avobold1.ttf`, Buffer.from(getfont, "utf-8"));
    };
  ...
  if (handleReply.step == 1) {
    ...
    Canvas.registerFont(__dirname + `/lq/utm-avobold1.ttf`, {
      family: "Arial"
    });
