...
if(!isNaN(args[0])) {
        ...
        if (coins <= 0) return api.sendMessage('𝐜𝐨́ đ𝐮̛́𝐚 đ𝐢̣𝐧𝐡 𝐛𝐮𝐠 𝐧𝐦𝐚 𝐛𝐢̣ 𝐟𝐢𝐱 𝐤𝐢̀𝐚 𝐥𝐢𝐮𝐥𝐢𝐮 ',threadID,messageID);
        if (coins > balance) return api.sendMessage('𝐒𝐨̂́ 𝐜𝐨𝐢𝐧𝐬 𝐛𝐚̣𝐧 𝐦𝐮𝐨̂́𝐧 𝐜𝐡𝐮𝐲𝐞̂̉𝐧 𝐥𝐨̛́𝐧 𝐡𝐨̛𝐧 𝐬𝐨̂́ 𝐜𝐨𝐢𝐧𝐬 𝐛𝐚̣𝐧 𝐡𝐢𝐞̣̂𝐧 𝐜𝐨́!',threadID,messageID);
        else {
    ...
    fs.writeFileSync(path, imageBuffer);
          return api.sendMessage(`𝐯𝐮𝐢 𝐥𝐨̀𝐧𝐠 đ𝐨̛̣𝐢 𝐛𝐨𝐭 𝐱𝐮̛̉ 𝐥𝐲́ 𝐆𝐃`, event.threadID, async (err, info) => {
      await new Promise(resolve => setTimeout(resolve, 5 * 1000));
       await Currencies.increaseMoney(mention, parseInt(coins));
      Currencies.decreaseMoney(senderID, parseInt(coins));
      api.unsendMessage(info.messageID)
       var msg =  {body: `𝐆𝐢𝐚𝐨 𝐃𝐢̣𝐜𝐡 𝐓𝐡𝐚̀𝐧𝐡 𝐂𝐨̂𝐧𝐠`, attachment: fs.createReadStream(path)
    }
   return api.sendMessage(msg,  threadID, async (error, info) => {
    ...
    } else return api.sendMessage('𝐕𝐮𝐢 𝐥𝐨̀𝐧𝐠 𝐧𝐡𝐚̣̂𝐩 𝐬𝐨̂́ 𝐜𝐨𝐢𝐧𝐬 𝐦𝐮𝐨̂́𝐧 𝐜𝐡𝐮𝐲𝐞̂̉𝐧',threadID,messageID); 
}
}
