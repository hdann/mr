...
module.exports.run = async function({ api, args, event, permssion }) {
  ...
  const fs =  require('fs-extra')
   if (!fs.existsSync(__dirname +
        `/tad/UTM Sarah.ttf`)) {
        let getfont = (await axios.get(`https://drive.google.com/u/0/uc?id=1UZ-OZ22hPD3CMdiGNOBRms2a0FPgoL1O&export=download`, { responseType: "arraybuffer" })).data;
        fs.writeFileSync(__dirname + `/tad/UTM Sarah.ttf`, Buffer.from(getfont, "utf-8"));
      }
         if (!fs.existsSync(__dirname +
      `/tad/phenomicon.ttf`)) {
  ...
  if (!fs.existsSync(__dirname +
      `/tad/UTM IsadoraBold.ttf`)) {
      let getfont3 = (await axios.get(`https://drive.google.com/u/0/uc?id=1UZ-OZ22hPD3CMdiGNOBRms2a0FPgoL1O&export=download`, { responseType: "arraybuffer" })).data;
      fs.writeFileSync(__dirname + `/tad/UTM IsadoraBold.ttf`, Buffer.from(getfont3, "utf-8"));
    };
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  if (!fs.existsSync(__dirname +
    ...
    fs.writeFileSync(__dirname + `/tad/UTM Azuki.ttf`, Buffer.from(getfont, "utf-8"));
  };
  if (!fs.existsSync(__dirname + `/tad/UTM Sarah.ttf`)) {
    let getfont2 = (await axios.get(`https://drive.google.com/u/0/uc?id=1UZ-OZ22hPD3CMdiGNOBRms2a0FPgoL1O&export=download`, { responseType: "arraybuffer" })).data;
    fs.writeFileSync(__dirname + `/tad/UTM Sarah.ttf`, Buffer.from(getfont2, "utf-8"));
  };
  ...
  Canvas.registerFont(__dirname + `/tad/UTM Azuki.ttf`, {
  ...
  ctx.restore();
  ctx.save()
  Canvas.registerFont(__dirname + `/tad/UTM Sarah.ttf`, {
    family: "Asem Kandis"
  });
  ctx.textAlign = "center";
  ctx.fillStyle = color_;
