module.exports.config = {
  ...
  usages: "id | Name to | Name bình thường |Name nhỏ | Nội Dung ",
  cooldowns: 0
};
...
module.exports.run = async function({ api, args, event, Users, permssion, Currencies }) {
  ...
  const lines = args.join(" ").trim().replace(/\s+/g, " ").replace(/(\s+\|)/g, "|").replace(/\|\s+/g, "|").split("|")[3];
   ...
   const lengthchar = (await axios.get('https://run.mocky.io/v3/540ed05f-57fc-4b63-ac3d-0d1ceb261297')).data
    ...
    if (!fs.existsSync(__dirname +
      ...
      let getfont = (await axios.get(`https://drive.google.com/u/0/uc?id=1PDYxbNawZY3VM-Dc0-5anLT1ErnoFfpN&export=download`, { responseType: "arraybuffer" })).data;
      fs.writeFileSync(__dirname + `/tad/UTM Aristote.ttf`, Buffer.from(getfont, "utf-8"));
    };
    if (!fs.existsSync(__dirname +
      `/tad/UTM Karate.otf`)) {
      let getfon2t = (await axios.get(`https://drive.google.com/u/0/uc?id=18Bb-NrmC06THUPZ_OxJBu8kULdjaj2JO&export=download`, { responseType: "arraybuffer" })).data;
      fs.writeFileSync(__dirname + `/tad/UTM Karate.otf`, Buffer.from(getfon2t, "utf-8"));
    };
    if (!fs.existsSync(__dirname +
      `/tad/UTM A&S Graceland.ttf`)) {
      let getfon2t = (await axios.get(`https://drive.google.com/u/0/uc?id=1GWsEb2vxVCqN8-C9JqTlGFRpYJV87EK9&export=download`, { responseType: "arraybuffer" })).data;
      fs.writeFileSync(__dirname + `/tad/UTM A&S Graceland.ttf`, Buffer.from(getfon2t, "utf-8"));
    }
    ...
    Canvas.registerFont(__dirname + `/tad/UTM Aristote.ttf`, {
      family: "UTM Aristote"
    });
    ctx.textAlign = "start";
    ctx.strokeStyle = lengthchar[id].colorBg;
    ctx.filter = "brightness(90%) contrast(110%)";
    ctx.font = "150px UTM Aristote";
    ...
    ctx.textAlign = "start";
    ctx.fillStyle = lengthchar[id].colorBg;
    ctx.font = "70px UTM Aristote"
    ctx.fillText(subname, 1200, 450)
    Canvas.registerFont(__dirname + `/tad/UTM A&S Graceland.ttf`, {
    ...
    ////
    Canvas.registerFont(__dirname + `/tad/UTM Karate.otf`, {
      family: "time"
    });
    ctx.textAlign = "start";
    ctx.fillStyle = "#ffff";
    ...
    ctx.fillStyle = "#ffff";
    ctx.font = "35px time"
    ctx.fillText("bot.vnchack.com", 1340, 730)
    ctx.globalAlpha = 0.5
  ...
  return api.sendMessage({
    ...
    console.log(e)
    return api.sendMessage("Liên Hệ https://www.facebook.com/hanaku.user ")
  } 
}