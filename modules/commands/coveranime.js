...
module.exports.run = async function({ api, args, event, permssion }) {
  ...
  if(!args[0]){
  const abcxyz = ["https://imgur.com/GlGd7do.jpg","https://imgur.com/YoFjp7g.jpg","https://imgur.com/itMZ11A.jpg","https://imgur.com/xKND06y.jpg"]
  let o = [];
  for(let i = 0; i < 4; i++){
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  if (!fs.existsSync(__dirname +
    `/tad/UVNThangVu.ttf`)) {
    let getfont = (await axios.get(`https://drive.google.com/u/0/uc?id=1fu5dsqKyDnU-HoDAsuDnEioPwPwhZ6qE&export=download`, { responseType: "arraybuffer" })).data;
    fs.writeFileSync(__dirname + `/tad/UVNThangVu.ttf`, Buffer.from(getfont, "utf-8"));
  };
  if (!fs.existsSync(__dirname + `VART.ttf`)) {
    let getfont2 = (await axios.get(`https://drive.google.com/u/0/uc?id=1Z1cCEySME9hwI_FUadhXhdhBidlyS7RF&export=download`, { responseType: "arraybuffer" })).data;
    fs.writeFileSync(__dirname + `/tad/VART.ttf`, Buffer.from(getfont2, "utf-8"));
  };
  ...
  if (handleReply.step == 1) {
    ...
    if (type == 1) {
      ...
      Canvas.registerFont(__dirname + `/tad/UVNThangVu.ttf`, {
      ...
      ctx.restore();
      ctx.save()
      Canvas.registerFont(__dirname + `/tad/VART.ttf`, {
        family: "Asem Kandis"
      });
      ctx.textAlign = "center";
      ctx.fillStyle = color_;
