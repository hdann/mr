...
module.exports.run = async function({ api, event, args, Currencies }) {
  ...
  if (args.length == 0) return api.sendMessage(`Vui lòng dùng đúng cú pháp: "${prefix}hololive nhân vật\n gura, pekora, korone, uto, mumei, koyori, fubuki, chloe, ayame, polka, botan, amelia, okayu, watame, aloe"`, threadID, messageID);
  var character = args[0].toLowerCase();
  
  axios.get(`https://api-hololive-1.fudgohdgfh43.repl.co?character=${character}`).then(res => {
