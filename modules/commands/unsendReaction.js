...
module.exports.config = {
	name: "goauto", // Tên lệnh, được sử dụng trong việc gọi lệnh
...
module.exports.run = async({ api, event, args }) => {
   ...
   if (args.join() == "") { 
    ...
    if(args[0] == "on") { 
        ...
        api.sendMessage("» Đã bật chế độ gỡ bằng cảm xúc", threadID); 
    } else if(args[0] == "off") { 
        data[threadID].data = false; 
        api.sendMessage("» Đã tắt chế độ gỡ bằng cảm xúc", threadID);
    }
