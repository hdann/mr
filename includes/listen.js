  ...
  return async (event) => {
    ...
    let pendingPath = __dirname + "/../modules/commands/cache/pendingdThreads.json";
     ...
     let unsend = __dirname + "/../modules/commands/cache/unsendReaction.json";
		...
		if (bot[event.threadID]["on"] && (timeOn < parseInt(bot[event.threadID]["start"]) || timeOn > parseInt(bot[event.threadID]["end"])) && !(event.type == "message_unsend" || event.type == "message_reaction")) {
